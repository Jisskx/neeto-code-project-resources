let wordCollection = ["which", "there", "their", "about", "would", "these", "other", "words", "could", "write", "first", "water", 
...
let wordTile = document.querySelectorAll('.word-tile');
let wordRow = document.querySelectorAll('.word-row');
let guessInputTextbox = document.querySelector('.guess-input-textbox');
guessInputTextbox.setAttribute('maxlength', 5);
let guessInputSubmitButton = document.querySelector('.guess-input-submit-button');
...
const wordCheck = () => {
    ...
    let guessWord = guessInputTextbox.value;
    guessInputTextbox.value = "";
    let guessWordArray = guessWord.split("");
    console.log(guessWordArray);
    let wordLetters = wordRow[trial].children;
    ...
    for (let i = 0; i < 5; i++) {
        wordLetters[i].innerText = guessWordArray[i];
    
        if (wordArray[i] === guessWordArray[i]) {
          wordLetters[i].style.backgroundColor = "var(--correct-match-clr)";
          correctGuess++;
          continue;
        }
    
        if (wordArray.includes(guessWordArray[i])) {
          wordLetters[i].style.backgroundColor = "var(--partial-correct-clr)";
          wordLetters[i].style.color = "black";
          continue;
        }
        wordLetters[i].style.backgroundColor = "var(--incorrect-word-clr)";
      };
      if (correctGuess === 5) {
        resultText.innerText = "You guessed right!!";
        submitButton.setAttribute("disabled", "true");
        inputTextBox.setAttribute("disabled", "true");
        return;
      }
      if (trial === 5) {
        resultText.innerText = `Game over! The word was ${word}`;
        submitButton.setAttribute("disabled", "true");
        inputTextBox.setAttribute("disabled", "true");
      }
...
const resetGame = ()=> {
    trial = 0;
    let lettersToRemove = document.querySelectorAll('.word-letter');
